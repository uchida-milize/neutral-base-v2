...
// XXX 専用トークンを読み込む。.acme-scope 配下でのみ shadcn セマンティック層が
 ...
 * /acme/* 全ページ共通のラッパー。
 *
 * - <div className="acme-scope"> でラップすることで、子孫の shadcn 系
 *   コンポーネントが自動的に XXX 色 (teal primary 等) を吸い込む
 * - SiteHeader もこのラッパー内に置くので、ヘッダーの primary 色も
 *   XXX 仕様に追従する
 * - 顧客視点: XXX 担当者には /acme/ 配下の URL だけを案内し、
...
export default function XxxLayout({
  ...
  return (
    <div className="acme-scope min-h-screen bg-background text-foreground transition-colors duration-300 ease-out">
      <SiteHeader />
      {children}
    </div>
  );
